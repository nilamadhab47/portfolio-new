  ...
  return (
    ...
    >
      ...
      <div className="absolute top-24 right-8 md:right-16 animate-float">
        <div className="bg-th-surface border border-green-500/30 px-4 py-2 font-mono text-xs">
      ...
      <RevealOnScroll>
        <div className="max-w-5xl" style={getDrunkStyle()}>
          {/* Intro */}
          <p className="text-th-text-muted font-mono text-sm md:text-base mb-6 tracking-wide">
            Hello, World! I'm
          </p>

          {/* BIG NAME */}
          <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-bold tracking-tighter mb-4 leading-[0.9]">
            <span className="block text-stroke hover:text-th-text transition-all duration-300 cursor-default hoverable">
              NILAMADHAB
            </span>
            <span className="block text-th-text">
              SENAPATI
            </span>
          </h1>
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <span className="text-2xl md:text-3xl lg:text-4xl font-light text-th-text-sub">
              Backend & Cloud Engineer
            </span>
          </div>

          {/* Fun description */}
          <p className="text-th-text-dim text-lg md:text-xl max-w-2xl mb-12 leading-relaxed">
            I mass destroy{" "}
            ...
            for a living.{" "}
            <span className="text-th-text-dim">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
            <div className="group hoverable cursor-default">
              <div className="text-4xl md:text-5xl font-bold text-th-text group-hover:animate-glitch">
                200K<span className="text-green-500">+</span>
              </div>
              <div className="text-th-text-muted text-sm font-mono mt-1">
                users served
              </div>
            </div>
            <div className="group hoverable cursor-default">
            ...
            >
              <div className={`text-4xl md:text-5xl font-bold text-th-text group-hover:animate-glitch group-hover:text-yellow-400 transition-colors ${drunkLevel > 10 ? 'animate-pulse' : ''}`}>
                {beerCount}<span className="text-yellow-500">🍺</span>
              </div>
              <div className="text-th-text-muted text-sm font-mono mt-1 group-hover:text-yellow-500/50 transition-colors">
                {clickStreak > 0 ? (
              ...
              {drunkLevel > 0 && (
                <div className="mt-2 h-1 bg-th-hover rounded-full overflow-hidden">
                  <div 
          ...
          <div className="flex flex-wrap gap-6">
            <a
              href="#projects"
              className="group hoverable inline-flex items-center gap-3 text-th-text text-lg font-medium hover-line"
            >
              See my work
            ...
            <a
              href="#contact"
              className="hoverable inline-flex items-center gap-3 text-th-text-muted text-lg font-medium hover:text-th-text transition-colors hover-line"
            >
              Let's grab a drink 🍻
            </a>
          </div>
      ...
      {/* Bottom left - time */}
      <div className="absolute bottom-8 left-8 font-mono text-xs text-th-text-faint hidden md:block">
        {time.toLocaleTimeString('en-US', { hour12: false })} IST
      </div>

      {/* Bottom right - scroll hint */}
      <div className="absolute bottom-8 right-8 font-mono text-xs text-th-text-faint flex items-center gap-2 hidden md:flex">
        <span>scroll</span>
